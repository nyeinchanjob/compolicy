...
MyApp.controller('ProductCtrl', ['$scope', '$mdDialog', '$http', function($scope, $mdDialog, $http) {
    	...
    	$scope.showProductDetail = function() {
      		$mdDialog.show({
        		...
        		templateUrl: 'templates/product/product_detail.html',
						locals: {id : undefined, action : 'new'},
						scope: $scope,
						preserveScope: true,
	...
	$scope.getAll = function() {
		$http.post('db_controller/product/read_product.php').success( function (response) {
	...
	$scope.delete = function() {
		if (confirm("Are you sure want to delete?")) {
			$http.post('db_controller/product/delete_product.php', {'id' : $scope.cbselected
...
function DialogController($scope, $http, $mdDialog, id, action) {
	if (id != undefined) {
		$http.post('db_controller/product/read_one_product.php', {'id' : id
	}).success(function(data, status, headers, config) {
		$scope.productInfo = {
